...
export default function PLRating() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(err?.response?.data?.error || 'Failed to load rating overview');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container py-4">
      <h2 className="mb-3">Program Leader Rating</h2>
      <Card>
        <Card.Body>
          {courses.length === 0 ? (
            <div className="text-muted">No courses to display</div>
          ) : (
